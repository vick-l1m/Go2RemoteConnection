...
/*
 * Vendored + locally patched from urdf-loader@0.13.1 (src/URDFLoader.js).
 * Dropped the stock STLLoader/ColladaLoader static imports and the matching
 * branches in defaultMeshLoader() below: this app's robot_model/ assets are
 * exclusively .glb (see tools/gen_robot_model_assets.py), loaded via a custom
 * loadMeshCb (see robot_viewer.js), so those loaders are dead weight here and
 * would otherwise need vendoring too. Re-diff against a newer urdf-loader
 * release rather than hand-patching further if this needs to change.
 */
...
class URDFLoader {
    ...
    // Default mesh loading function. STL/Collada support was dropped in this
    // vendored copy (see the header comment) — this app always supplies its
    // own loadMeshCb (robot_viewer.js) that loads .glb via GLTFLoader, so this
    // is only reached if that wiring is missing.
